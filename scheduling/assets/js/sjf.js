...
const play = (event) =>{
    ...
    let ct =processObject[0].bt
    for(let i =1; i<processObject.length;i++){
        ct += processObject[i].bt
        for(let j=i+1;j<processObject.length;j++){
            if(processObject[i].bt>processObject[j].bt && processObject[j].at<=ct){
                let tmp = processObject[i]
                processObject[i] = processObject[j]
                processObject[j] = tmp;
            }
        }
    }
